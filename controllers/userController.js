...
const registerUser = async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.status(400).json({ success: false, message: "User alreay exists" })
        }
        // create new user
...
const loginUser = async (req, res) => {
    try {
        ...
        if (!existingUser) {
            return res.status(400).json({ success: false, message: "User Dosen't exist" })
        }
...
const uploadAvatar = async (req, res) => {
    try {
        const loggedUser = await User.findById(req.user._id);
        if (!loggedUser) {
            return res.status(400).json({ success: false, message: "Please sign in first" })
        }
...
const removeProfilePicture = async (req, res) => {
    try {
        ...
        const removedAvatar = await User.findByIdAndUpdate(req.user._id, {
            $unset: { avatar: "" },
            $set: { isAvatarSet: false }
        }, { new: true })
        if (!removedAvatar) {
            return res.status(400).json({ success: false, message: "Cannot remove, please try after sometime" })
        } else {
...
const getMe = async (req, res) => {
    try {
        ...
        let { _id, name, email, avatar, isAvatarSet, groupNotifications } = await User.findById(req.user._id).populate({ path: 'groupNotifications', options: { sort: { updatedAt: -1 } } })
        groupNotifications = await User.populate(groupNotifications, {
            path: "chat",
            select: "_id groupName groupImage isGroupChat"
        })
        res.status(200).json({ success: true, _id, name, email, avatar, isAvatarSet, groupNotifications })
    } catch (error) {
        res.status(500).json({ success: false, message: error.message })
    }
}
...
const loadGroupNotifications = async (req, res) => {
    try {
        let notifications = await User.find({ _id: req.user._id }).select('groupNotifications').populate({ path: 'groupNotifications', options: { sort: { updatedAt: -1 } } })
...
module.exports = {
    registerUser, loginUser, updatePassword, updateProfile, searchUsers, uploadAvatar, getMe, removeProfilePicture, loadGroupNotifications
}